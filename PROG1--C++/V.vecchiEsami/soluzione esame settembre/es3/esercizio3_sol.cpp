...
double calcola(Employee * emp) {
    if (!emp) return 0.0; // If the employee is null, return 0

    double totalCost = emp->salary + emp->projectBudget; // Start with salary and project budget

    // Recursively calculate the cost of direct reports
    Employee* currentReport = emp->directReports;
    while (currentReport) {
        totalCost += calcola(currentReport);
        currentReport = currentReport->nextReport; // Move to the next direct report
    }

    return totalCost; // Return the total cost for this employee
}
